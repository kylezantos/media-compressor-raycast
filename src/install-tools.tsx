...
export default async function InstallTools() {
  await showToast({ style: Toast.Style.Animated, title: "Installing compression tools..." });

  try {
    await installAllTools();
    await showHUD("Installed: pngquant, oxipng, jpegoptim, ffmpeg, compress-overlay");
  } catch (err) {
    await showToast({
